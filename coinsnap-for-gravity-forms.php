 ...
 * Plugin URI:      https://coinsnap.io/wp-add-ons/gravity-forms/
...
if(!defined('COINSNAPGF_VERSION')){ define( 'COINSNAPGF_VERSION', '1.3.5' ); }
...
// Setting up and handling custom endpoint for api key redirect from BTCPay Server.
    add_rewrite_endpoint('coinsnap-for-gravity-forms-btcpay-settings-callback', EP_ROOT);
});

// To be able to use the endpoint without appended url segments we need to do this.
add_filter('request', function($vars) {
    if (isset($vars['coinsnap-for-gravity-forms-btcpay-settings-callback'])) {
        $vars['coinsnap-for-gravity-forms-btcpay-settings-callback'] = true;
        $vars['coinsnap-for-gravity-btcpay-nonce'] = wp_create_nonce('coinsnapgf-btcpay-nonce');
    }
    return $vars;
});